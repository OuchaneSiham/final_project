...
export default function ChatPage({ params }) {
  ...
  // Fetch messages when page loads or conversation changes
  useEffect(() => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/conversation/${conversationId}/message`,
        {
          ...
          body: JSON.stringify({ userId: USER_ID, content: messageText })
        }
      )
    ...
    <div style={{ padding: 20 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        {messages.map(m => {
          ...
          const username = isMe ? "You" : (m.sender && m.sender.username) || `User ${m.senderId}`
